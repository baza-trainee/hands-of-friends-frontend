...
export default function Modal ({ lng, handleClose  }) {
    const { t } = useTranslation(lng, "modal");
    return (
        // <div className="flex items-center justify-center rounded bg-lavender w-[503px] h-[618px] z-10 relative">
        <div className="md:justify-self-end justify-self-center flex md:mt-20 mt-[60%] 2xl:w-[527px] xl:w-[503px]  md:w-[334px]  sm:w-[340px] xs:w-[268px] items-center bg-lavender shadow-md z-10 justify-center relative h-[618px] text-center rounded">
            <div className="flex flex-col items-center gap-10 text-xl">
                ...
                <HiPlus size={28} className="absolute transition transform rotate-45 cursor-pointer top-5 fill-slate-900 right-5 hover:fill-violet" onClick={handleClose} />
            </div>
        </div >
    )
}
