		...
		<div className='fixed z-20 w-full border-t border-b bg-lightBlue border-deepBlue'>
			<div className='flex xl:py-4 mx-auto max-w-screen-2xl 2xl:px-[120px] xl:px-20 md:px-10 px-4 relative xl:flex-row flex-col xl:gap-4 xl:items-center xl:justify-between gap-0'>
				...
				<div className='flex items-center justify-between w-full gap-5 p-4 sm:py-5 md:py-[22px] xl:hidden'>
					...
					target='_blank'
					rel='noopener noreferrer'
					className='hidden p-4 text-xl text-white transition border-2 rounded cursor-pointer xl:self-center bg-violet border-violet hover:bg-transparent hover:text-violet md:block xl:hidden'
				>
					{t('support')}
				</a>
				</div>
				...
				>
					{menuOpen && (
						<div className='flex flex-col gap-4 px-4 xl:flex-row xl:justify-between'>
							<div className='bg-deepBlue h-[1px] w-full'></div>
							<div className='flex items-center xl:justify-center'>
								<ul className='flex flex-col justify-between gap-4 mb-4 xl:gap-14 xl:flex-row'>
									{links.map((link) => (
										<li key={link.id}>
											<Link
												href={link.url}
							...
							<a
								...
								target='_blank'
								rel='noopener noreferrer'
								className='hidden px-4 py-2 mb-4 text-sm text-white transition border-2 rounded cursor-pointer border-violet xl:self-center bg-violet w-36'
							>
								{t('support')}
							</a>
						</div>
